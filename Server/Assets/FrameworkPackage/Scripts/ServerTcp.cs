...
public class ServerTcp : MonoBehaviour
{
    ...
    public void changeCurrentClient()
    {
        ...
        int noClients = connectedClients.Count();
        /* Cannot change current client if you have maximum 1 client. */
        if(noClients <= 1)
    {
        Debug.Log("Pressed C: cannot change current client.");
        return;
    }
    /* Change the counter in the connected client list. */
    else {
        Debug.Log("Pressed C: client changed.");
        clientCounter += 1;
        clientCounter = clientCounter % connectedClient.Count();
        connectedTcpClient = connectedClients[clientCounter];
        }
    }
